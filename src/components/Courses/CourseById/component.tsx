...
export default function CourseByIdComponent({ id }: CoursesByIdComponentProps) {
  const [requestCourseById, isLoadingCourseById, courseById] =
    useFetch<Course>();

  const isBreaking = useBreakpointValue({
    base: true,
    md: false,
  });
  ...
  //   );
  //   return (
  //     (totalEvaluation ?? 0) / (courseById?.reviews?.length || 1)
  //   ).toFixed(1);
  // };

  const handleGetCourseById = async () => {
    return await requestCourseById(`/api/courses/${id}`, {
      method: "GET",
    });
  };
          ...
          >
            ...
            {isBreaking && (
              ...
              >
                ...
                <Button variant={"custom"}>+ Adicionar ao Carrinho</Button>
                {/* <CardInfos
            icon={FaRegStar}
